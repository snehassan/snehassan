...
- **Software Developer Intern at Telvue** - Built an event-driven notification system (Rails + Firebase) increasing mobile engagement by 40%

### Tech I work with
`Java` `Python` `TypeScript` `C` `React` `Next.js` `Spring Boot` `Flask` `Node.js` `PostgreSQL` `MongoDB` `Redis` `AWS` `GCP` `Firebase` `Docker` `PyTorch` `TensorFlow` `OpenCV` `LangChain`
